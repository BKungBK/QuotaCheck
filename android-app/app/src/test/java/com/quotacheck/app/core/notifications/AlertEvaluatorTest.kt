...
class AlertEvaluatorTest {
    ...
    @Test fun `emits failure exactly at three and suppresses until recovery`() {
        ...
        assertEquals(emptyList<AlertCommand>(), evaluator.commands(emptyList(), emptyList(), SyncResult.Failed(com.quotacheck.app.core.model.FailureCategory.RETRYABLE), SyncTrigger.PERIODIC, preferences, 1))
        assertEquals(listOf(AlertCommand.SyncFailure(3)), evaluator.commands(emptyList(), emptyList(), SyncResult.Failed(com.quotacheck.app.core.model.FailureCategory.RETRYABLE), SyncTrigger.PERIODIC, preferences, 2))
        assertEquals(emptyList<AlertCommand>(), evaluator.commands(emptyList(), emptyList(), SyncResult.Failed(com.quotacheck.app.core.model.FailureCategory.RETRYABLE), SyncTrigger.PERIODIC, preferences, 3))
    ...
    @Test fun `success is only manual or recovery when enabled`() {
        ...
        assertEquals(emptyList<AlertCommand>(), evaluator.commands(emptyList(), emptyList(), SyncResult.Success(Instant.EPOCH, 0), SyncTrigger.PERIODIC, preferences, 0))
        assertEquals(listOf(AlertCommand.SyncSuccess(0)), evaluator.commands(emptyList(), emptyList(), SyncResult.Success(Instant.EPOCH, 0), SyncTrigger.MANUAL, preferences, 0))
        assertEquals(listOf(AlertCommand.SyncSuccess(2)), evaluator.commands(emptyList(), emptyList(), SyncResult.Success(Instant.EPOCH, 0), SyncTrigger.PERIODIC, preferences, 2))
    }
